...
class TestTransaction(unittest.TestCase):
  ...
  def test_get_action(self):
    v4_transaction = json.loads(Path('%s/../testdata/withdraw_delegator_reward_v4.json' % os.path.dirname(__file__)).read_text())
    transaction = Transaction(v4_transaction)
    self.assertEqual(transaction.get_action(), 'withdraw_delegator_reward')

    v3_transaction = json.loads(Path('%s/../testdata/withdraw_delegator_reward_v3.json' % os.path.dirname(__file__)).read_text())
    transaction = Transaction(v3_transaction)
    self.assertEqual(transaction.get_action(), 'withdraw_delegator_reward')
